  ...
  if (notes.length === 0) {
    return (
      <div className="empty-state">
        <p className="empty">No notes captured.</p>
        <p className="empty-hint">Capture your thoughts, ideas, and musings.</p>
      </div>
    );
  }
      ...
      <ul className="note-grid">
        {notes.map((note) => (
          ...
          >
            ...
            <div className="row-actions mt-auto pt-2">
              <Button type="button" variant="ghost" size="sm" className="h-8 px-2 text-muted-foreground hover:text-foreground" onClick={() => startEdit(note)}>
                <Pencil size={14} />
                Edit
              </Button>
              <Button type="button" variant="ghost" size="sm" className="h-8 px-2 text-muted-foreground hover:text-foreground" onClick={() => pinNote(note.id)}>
                <Pin size={14} />
                {note.pinned ? "Unpin" : "Pin"}
              </Button>
              <Button type="button" variant="ghost" size="sm" className="h-8 px-2 text-muted-foreground hover:text-destructive" onClick={() => onDelete(note.id)}>
                <Trash2 size={14} />
                Delete
              </Button>
            </div>
